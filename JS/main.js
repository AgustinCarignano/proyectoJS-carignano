...
contadorCarrito.innerText = carrito.length === 0 ? "" : carrito.length;
  ...
  document.querySelector("#aplicarFiltro").addEventListener("click", () => {
    ...
    for (const categoria of categorias) {
      if (categoria.checked) {
        let categoriaFiltrada = tienda.filter(
          (el) => el.categoria == categoria.id
        );
  ...
  array.forEach((producto) => {
    document
      ...
      .addEventListener("click", () => {
        carrito.some((prod) => prod.id === producto.id)
          ? carrito.find((prod) => prod.id === producto.id).cantidad++
          : ((producto.cantidad = 1), cargar(carrito, producto));
        ...
        Toastify({
          ...
          onClick: () => {
            crearCarrito(1000);
          }, //Para ver el carrito de compras al clickear sobre el mensaje
        }).showToast();
      });
  });
}
...
function agregarBotonRestar() {
  carrito.forEach((producto) => {
    document
      .querySelector(`#btn-restar${producto.id}`)
      .addEventListener("click", () => {
        producto.cantidad > 1
...
function agregarBotonSumar() {
  carrito.forEach((producto) => {
    document
      .querySelector(`#btn-sumar${producto.id}`)
      .addEventListener("click", () => {
        producto.cantidad = producto.cantidad + 1;
        crearCarrito(0);
      });
...
function agregarFuncionBotonAtrasEnvio() {
  botonAtrasEnvio = document
    .querySelector("#botonAtrasEnvio")
    .addEventListener("click", () => {
      divComprar.innerHTML = "";
      crearCarrito(500);
    });
...
function agregarFuncionBotonSiguienteEnvio() {
  botonSiguienteEnvio = document
    .querySelector("#botonSiguienteEnvio")
    .addEventListener("click", () => {
      const radioBotones = document.querySelectorAll(
        'input[name="formaEnvio"]'
      );
...
function agregarFuncionBotonAtrasDireccion() {
  botonAtrasDireccion = document
    .querySelector("#botonAtrasDireccion")
    .addEventListener("click", () => {
      formaEnvio();
    });
}
...
function agregarFuncionBotonSiguienteDireccion() {
  botonSiguienteDireccion = document
    .querySelector("#botonSiguienteDireccion")
    .addEventListener("click", () => {
...
function agregarFuncionBotonAtrasPago() {
  botonAtrasPago = document
    .querySelector("#botonAtrasPago")
    .addEventListener("click", () => {
      datosEnvio();
    });
}
...
function agregarFuncionBotonSiguientePago() {
  botonSiguientePago = document
    .querySelector("#botonSiguientePago")
    .addEventListener("click", () => {
...
function agregarFuncionAtrasTarjeta() {
  botonAtrasTarjeta = document
    .querySelector("#botonAtrasTarjeta")
    .addEventListener("click", () => {
      metodoPago();
    });
}
...
function agregarFuncionSiguienteTarjeta() {
  botonSiguienteTarjeta = document
    .querySelector("#botonSiguienteTarjeta")
    .addEventListener("click", () => {
...
function resumenCompra() {
  ...
  seleccionEnvio == "domicilio"
    ? (costoEnvio = "$750")
    : (costoEnvio = "Sin cargo");
  seleccionEnvio == "domicilio"
    ? (valorTotal = 750 + subTotal)
    : (valorTotal = subTotal);
  seleccionPago == "efectivo"
    ? (formaPago = "Al retirar la compra")
    : (formaPago = `Tarjeta terminada en ${numerosTarjeta}`);
...
function agregarFuncionAtrasResumen() {
  botonAtrasResumen = document
    .querySelector("#botonAtrasResumen")
    .addEventListener("click", () => {
      metodoPago();
    });
}
...
function agregarFuncionFinalizarCompra() {
  botonFinalizarCompra = document
    .querySelector("#botonFinalizarCompra")
    .addEventListener("click", () => {
      divComprar.innerHTML = `
